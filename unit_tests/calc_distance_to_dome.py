...
""" Place position at an arbitrary point in space and pick a random point on
the sphere.  The distance between the points and the direction from one to the
other can be easily calculated. """
...
for i in range(1000):
    ...
    if norm(result - expected_result) > required_accuracy:
        print "Mismatch at position:", position
        print "and direction:", direction
        print "Expected result:", expected_result
        print "Actual result:  ", result



print "Done with calc_distance_to_dome test"
