...
lower left quadrant of the camera's field of view.  Calculate the location of
the camera's optical center and the direction from it to the point on the dome.
Use this direction along with the camera's orientation to find the point's
pixel coordinates. Minimize camera_orientation_error to find the camera's
orientation and compare it to the randomly chosen orientation. """
...
for i in range(100):
    ...
    if norm(result - expected_result) > required_accuracy:
        print "Camera direciton mismatch!"
...
print "Done with camera_orientation_error test"
